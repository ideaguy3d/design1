...
<%	
	...
	sortorderby = ""
		...
		if showthispage=0 then
			...
			if REQUEST.FORM("ProductSearchBy") =1 then
				session("searchcriteria")= "PARTNUMBER"
				session("search_on") = "stock.number"	
			end if 
			if REQUEST.FORM("ProductSearchBy") =2 then
				...
				if SEARCH_ON_TITLE=1 then
				    build_searchcriteria = "STOCK.inetsdesc+"
				end if
...
<link rel="stylesheet" href="<%=secureurl%>text/topnav.css" type="text/css">
<link rel="stylesheet" href="<%=secureurl%>text/sidenav.css" type="text/css">
<link rel="stylesheet" href="<%=secureurl%>text/storestyle.css" type="text/css">
...
<div id="page-content">
  ...
  <%
			...
			&nbsp;&nbsp;
					<%if pcount>PRODUCTS_PER_PAGE and PRODUCTS_PER_PAGE>0 then 
									...
									if int(showthispage)>numberofpages then%>
										...
										<a HREF="searchprods.asp?searchstring=<%=server.urlencode(session("searchstring"))%>&amp;pagenumber=<%=nextprev%>&val=<%=session("val")%>">&nbsp;Previous&nbsp;</a><<&nbsp;&nbsp; <%	startpage=nextprev+numberofpages  'int(session("showthispage"))
									...
									<%if limitpages<totpages then%>
										&nbsp;&nbsp;&gt;&gt;<a HREF="searchprods.asp?searchstring=<%=server.urlencode(session("searchstring"))%>&amp;pagenumber=<%=nextnext%>&val=<%=session("val")%>"><font class="plaintextbold">&nbsp;More</font></a> 									
									<%end if%>
	...
	<table width="100%"  border="0" cellspacing="1" cellpadding="3">
		<tr>
			  <th align="CENTER" class="THHeader" width="15%">
					<span class="THHeader">Item #</span>
				</th>
			<th align="CENTER" class="THHeader" width="10%">Image</th>
			<th align="CENTER" class="THHeader" width="60%">
	...
	<%
		...
		'SLVariation  = SL_prod.item(x).selectSingleNode("variant").text
		 ...
		 SL_numcnt = SL_prod.item(x).selectSingleNode("numcnt").text
	...
	<tr>	
	<td class="<%=class_to_use%>" valign="middle" align="center"><a title="<%=SLProductTitle%>" class="allpage" HREF="<%=prodlink%>"><%=SLProdNumber%></a></td>
	<td class="<%=class_to_use%>" valign="middle" align="center"><a title="<%=SLProductTitle%>" HREF="<%=prodlink%>"><img SRC="<%=imagename%>" alt="<%=SLProductTitle%>" title="<%=SLProductTitle%>" <% if PROD_IMAGE_WIDTH > 0 then  response.write(" width="""& PROD_IMAGE_WIDTH&"""") end if %> <%if PROD_IMAGE_HEIGHT > 0 then response.write(" height="""& PROD_IMAGE_HEIGHT&"""") end if %> border="0"></a></td>
	<td class="<%=class_to_use%>" valign="top">
	<table width="100%" cellpadding="0" cellspacing="0" border="0">
	<tr><td><a title="<%=SLProductTitle%>" class="allpage" HREF="<%=prodlink%>"><%=SLProductTitle%></a></td></tr>
	...
	<% if SL_Pref_Ship <>"" then %>				
	<tr><td class="CompPrice">Will be shipped via : &nbsp; <%=SL_prefShip_Title%></td></tr>
	<%end if%>
...
<form enctype="application/x-www-form-urlencoded" method="post" action="itemadd.asp">
	...
	<%else%>	
		<%if has_Onespecial_Price=true then %>
			<span class="plaintext"><s><%=formatcurrency(SL_price)%></s></span>
			<br>
		...
		<%else%>
			<span class="plaintext"><%=formatcurrency(SL_price)%></span>
		<%end if %>

		  <%if SHOW_QTY=1 then %>
		  		<br><br>
				<table width="0" cellpadding="3" cellspacing="0" border="0">
				<tr><td align="center"><input type="hidden" name="additem" value="<%=SLProdNumber%>">	  				  		
