...
<html>
<head>
<title>Aussie Products.com | <%=trim(bookmarktitle)%>-<%=althomepage%> | Australian Products Co. Bringing Australia to You - Aussie Foods </title>
<script> 
  has_rewrite = <%=WANT_REWRITE%>
  if (has_rewrite==1)
  {
   ...
   var search_term = '.asp'; 
   var url_check = theURL.indexOf(search_term); 
   if (url_check > 0)
   {
    new_rewriteurl = "<%=insecureurl%>"+"productinfo/" + "<%=server.urlencode(current_stocknumber)%>" + "/" + "<%=usethis_for_urlwrite%>"    
    //document.write('<meta http-equiv="refresh" content="0;url='+new_rewriteurl+'"')    
    //document.write(new_rewriteurl)
	}
  } 
</script>
