	...
	for (j=vFirstDay; j<7; j++) {
		...
		if (sShowWeekend == 1)
			...
			if (vFlowDate < gNow){vCode = vCode + "<FONT COLOR='#808080'>" + vDay + "</FONT>";}
					...
					this.format_data(vDay) + "';window.close(); self.opener.document.basketform.submit();\">" + vDay +
				"</A></TD>";}
			}
		else
			{
			...
			if (sShowWeekend == 1)
				{
					...
					if(vFlowDate < gNow)
						...
						vCode = vCode + "<A  class='day'  HREF='#' onClick=\"self.opener.document." + this.gReturnItem + ".value='" +
							this.format_data(vDay) + "';window.close(); self.opener.document.basketform.submit();\">" + vDay + "</A>";
						}
					vCode = vCode + "</TD>";
				}
				...
				( (vFlowDate < gNow) ? "<FONT COLOR='#808080'>" + vDay + "</FONT>" : "<A  class='day'  HREF='#' onClick=\"self.opener.document." + this.gReturnItem + ".value='" +	this.format_data(vDay) + "';window.close(); self.opener.document.basketform.submit();\">" + vDay +	"</A>"));
				vCode = vCode + "</TD>";
...
/*
	...
	*/
	...
	vWinCal = window.open("", "CalendarDemo", "width=210,height=170,status=no,resizable=no,top=200,left=200");
	vWinCal.opener = self;
	vWinCal.focus();
	ggWinCal = vWinCal;
