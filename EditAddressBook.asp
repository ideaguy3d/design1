...
<%
  if session("registeredshopper")="NO" then
  	session("destpage")="EditAddressBook.asp"
...
<%
	...
	xmlAddressBookstring =sitelink.getaddressbook(session("addressbookcustnum"),shopcust_id)
	  ...
	  for z=0 to SL_AddressBook.length-1 
	  	 custnum		= (SL_AddressBook.item(z).selectSingleNode("custnum").text)
		 lname			= (SL_AddressBook.item(z).selectSingleNode("lastname").text)
		 fname			= (SL_AddressBook.item(z).selectSingleNode("firstname").text)
		 company		= (SL_AddressBook.item(z).selectSingleNode("company").text)
		 addr 		    = (SL_AddressBook.item(z).selectSingleNode("addr").text)
		 addr2 		    = (SL_AddressBook.item(z).selectSingleNode("addr2").text)
		 addr3 		    = (SL_AddressBook.item(z).selectSingleNode("addr3").text)
		 city 		    = (SL_AddressBook.item(z).selectSingleNode("city").text)
		 vstate 		= (SL_AddressBook.item(z).selectSingleNode("state").text)
		 zipcode 		= (SL_AddressBook.item(z).selectSingleNode("zipcode").text)
		 phone 		    = (SL_AddressBook.item(z).selectSingleNode("phone").text)
		 phone2		    = (SL_AddressBook.item(z).selectSingleNode("phone2").text)
		 email 		    = (SL_AddressBook.item(z).selectSingleNode("email").text)
		 countrycode     = (SL_AddressBook.item(z).selectSingleNode("country").text)
		 countycode     = (SL_AddressBook.item(z).selectSingleNode("county").text)

		 address_id		= (SL_AddressBook.item(z).selectSingleNode("address_id").text)		 
		 shopcust	    = (SL_AddressBook.item(z).selectSingleNode("shopcust").text)
...
<html>
<head>
<title>Aussie Products.com | Edit Address from Address Book | Australian Products Co.</title>
...
function validateForm(ShiptoEmailreq) {
 ...
 var checkforcountrydrop = document.getElementById('whatbcountrytype')
	...
	var semailrep = document.getElementById('txtsemail1').value; 
        ...
        if ((CheckForEmpty(semail) == false) && (semail != semailrep)) {
...
<div id="page-content">
	...
	<table width="100%"  cellpadding="3" cellspacing="0" border="0">
		<tr><td class="TopNavRow2Text" width="100%" >Modify Address for <%=fname%>&nbsp;<%=lname%> &nbsp;&nbsp; </td></tr>
	</table>
	<br>
	...
	<table width="100%"  border="0" cellspacing="0" cellpadding="2">
	  <tr><td><input type="hidden" name="address_id" value="<%=address_id%>">
		...
		<tr><td class="plaintextbold" align="right" width="160"> &nbsp;First Name&nbsp; </td>
			<td class="smalltextred"><input type="text" name="txtsfirstname" id="txtsfirstname" size="20,1" maxlength="15" value="<%=fname%>">&nbsp;*
			<span id="sfname"></span>
			</td>
		</tr>
		<tr>
            <td class="plaintextbold" align="right"> &nbsp;Last Name&nbsp; </td>
			<td class="smalltextred"><input type="text" name="txtslastname" id="txtslastname" size="30,1" maxlength="20" value="<%=lname%>">&nbsp;*
			<span id="slname"></span>
			</td>
		</tr>
		<tr><td class="plaintextbold" align="right"> &nbsp;Company&nbsp; </td>
			<td colspan="3"><input type="text" name="txtscompany" id="txtscompany" size="50,1" maxlength="40" value="<%=company%>"></td>
		</tr>
		<tr><td class="plaintextbold" align="right"> &nbsp;Address&nbsp; </td>
			<td colspan="3" class="smalltextred"><input type="text" name="txtsaddress1" id="txtsaddress1" size="50,1" maxlength="40" value="<%=addr%>">&nbsp;*
			<span id="saddr1"></span>
			</td>
		</tr>
		<tr><td class="plaintextbold" align="right"> &nbsp;Address 2&nbsp; </td>
			<td colspan="3"><input type="text" name="txtsaddress2" id="txtsaddress2" size="50,1" maxlength="40" value="<%=addr2%>"></td>
		</tr>		
		...
		<td class="plaintextbold" align="right"> Address 3 </td>
				<td ><input type="text" name="txtsaddress3" size="40,1" maxlength="40" value="<% =addr3 %>"></td>
		</tr>
		<tr><td class="plaintextbold" align="right"> &nbsp;City&nbsp; </td>
			<td colspan="3" class="smalltextred">
		    <input type="text" name="txtscity" id="txtscity" size="25,1" maxlength="20" value="<%=city%>">&nbsp;*
			<span id="scity"></span>
			</td>
		</tr>
		<tr id="txtsstatelabel" <%if show_saddr3=true then %>style="display:none"<%else%>style="display:''"<%end if%>>
							...
							<td class="smalltextred">
								<select name="txtsstate" id="txtsstate"  class="smalltextblk">
									  ...
									  <option value="">
									  <option value="INT" <%if vstate = "INT" then response.write(" selected") end if%>>INTERNATIONAL
									  <%
									  ...
									  <%if vstate = SL_state_code then response.write(" selected") end if%>
									  ><%=SL_state_name%>
									  <%next						  
							...
							<td class="plaintextbold" align="right">County</td>
								<td class="plaintext">
									 ...
									 <select name="txtscounty" id="txtscounty" class="smalltextblk">												 
												  ...
												  <%if countycode = SL_county_Code then response.write(" selected") end if%>
												  ><%=SL_county_name%>
												  <%next						  
        ...
        <tr><td class="plaintextbold" align="right"><span id="szipcodelabel"><%=txtsZiplabelTxt%></span>
							  <td class="smalltextred">
							  <input type="text" name="txtszipcode" id="txtszipcode" size="10,1" maxlength="10" value="<% =zipcode %>">&nbsp;*
							  <span id="szip"></span>
							</td>
						</tr>
    	...
    	<tr><td class="plaintextbold" align="right"> &nbsp;Phone&nbsp; </td>
			<td colspan="3"><input type="text" name="txtsphone" id="txtsphone" size="20,1" maxlength="14" value="<%=phone%>"></td>
		</tr>
		<tr><td class="plaintextbold" align="right"> &nbsp;E-Mail&nbsp; </td>
			<td colspan="3" class="smalltextred"><input type="text" name="txtsemail" id="txtsemail" size="50,1" maxlength="50" value="<%=email%>">
			<%if SHIP_TOEMAIL_REQUIRED=1 then response.write("&nbsp;*") end if%>
			<span id="semail"></span>
			</td>
		</tr>
		<tr><td class="plaintextbold" align="right"> &nbsp;Re-Enter E-Mail&nbsp; </td>
			<td colspan="3" class="smalltextred"><input type="text" name="txtsemail1" id="txtsemail1"  size="50,1" maxlength="50" value="<%=email%>">
	...
	<br>
	<input type="checkbox" name="deleteAddress" >&nbsp;
				Check this box if you want to remove this address from your address book.
	</td></tr>
		...
		<tr><td>&nbsp;<br><br></td>
			<td valign="bottom">
			<br>
			<input type="image" src="images/btn_update_address.gif" border="0" alt="Update Address Book">
			</td>
		</tr>
